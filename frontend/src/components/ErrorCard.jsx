...
const ErrorCard = ({ message, onBack, backLabel }) => {
  ...
  return (
    <Card
      className="my-4 sm:my-6 max-w-xl mx-auto p-4 sm:p-6 shadow-lg rounded-lg border border-red-300 bg-red-50"
      role="alert">
      <div className="text-center">
        <HiOutlineExclamationCircle className="mx-auto mb-4 h-12 w-12 sm:h-16 sm:w-16 text-red-500" />
        <h2 className="text-xl sm:text-2xl font-bold text-red-700 mb-2">Error</h2>
        <p className="text-sm sm:text-md text-red-600 mb-6 px-2">{resolvedMessage}</p>
        <Button
          color="light"
